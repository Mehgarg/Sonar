...
def getNewBoard():
    # Create a new 40x10 board data structure.
    board = []
    for x in range(40): # the main list is a list of 40 lists
        board.append([])
...
def getRandomChests(numChests):
    ...
    for i in range(numChests):
        chests.append([random.randint(0, 39), random.randint(0, 9)])
    return chests

def isValidMove(x, y):
    # Return True if the coordinates are on the board, otherwise False.
    return x >= 0 and x <= 39 and y >= 0 and y <= 9
...
def enterPlayerMove():
    ...
    print('Where do you want to drop the next sonar device? (0-39 0-9) (or type quit)')
    while True:
        move = input()
        if move.lower() == 'quit':
        ...
        print('Enter a number from 0 to 39, a space, then a number from 0 to 9.')
...
if input().lower().startswith('y'):
    showInstructions()
theChests = int(input('Enter the number of treasure cchests you want to find:'))
sonarDevices = int(input('Enter the number of sonar devices you will need:'))
